...
class Net(nn.Module):
    def __init__(self):                                                         # 定义Net的一系列属性
        ...
        self.fc1.weight.data.normal_(0, 1)                                      # 权重初始化 (均值为0，方差为1的正态分布)
        self.bn1 = nn.BatchNorm1d(1)
        self.fc2 = nn.Linear(128, 256)                                          # 设置第一个全连接层(输入层到隐藏层): 128个神经元到256个神经元
        self.fc2.weight.data.normal_(0, 1)                                      # 权重初始化 (均值为0，方差为1的正态分布)
        self.bn2 = nn.BatchNorm1d(1)
        self.fc3 = nn.Linear(256, N_ACTIONS)                                    # 设置第三个全连接层(隐藏层到输出层): 256个神经元到动作数个神经元
        self.fc3.weight.data.normal_(0, 1)                                      # 权重初始化 (均值为0，方差为1的正态分布)
...
class stock_market():
    ...
    def step(self,action):
        ...
        return self.state[self.data['trade_date'][self.curindex]],self.calc_reward(action)
    
    def calc_reward(self,action):
        # 根据action，当前的日期（index）和当前的持仓情况（pos）计算reward
        if self.pos == 0:
            if action == 0 or action == 1:
                return 100*(1-self.data['close'][self.curindex]/self.data['close'][self.curindex-horizon])
            else:
                return 100*((1-TC)*(self.data['close'][self.curindex]/self.data['close'][self.curindex-horizon])-1)
        
        else:
            if action == 0:
                return 100*((1-TC)*(2-self.data['close'][self.curindex]/self.data['close'][self.curindex-horizon])-1)
            else:
                return 100*(self.data['close'][self.curindex]/self.data['close'][self.curindex-horizon]-1)
...
model = DQN()
model.eval_net = torch.load('trained_dqn.pt')
...
while env.curindex < len(env.data) - horizon:
    ...
    # 第五天收盘时dqn模型给出的信号和6-10天对应的reward一并放在第十天那一行
    # 这里的reward不是真实收益，回测时无法做空，而且只能在第二天开盘时开仓or平仓，只是为了训练模型给智能体的收益，起参考作用
    s = s_
